...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private bool ValidConfig
        {
            get
            {
                ...
                if (pagination != null)
                {
                    ...
                    {
                        // Chỉ cảnh báo, không block nếu NumberOfPanels = 0 (có thể được tạo động sau)
                        if (NumberOfPanels > 0)
                        {
                            Debug.LogWarning("<b>[SimpleScrollSnap]</b> The number of Toggles should be equivalent to the number of Panels. There are currently " + numberOfToggles + " Toggles and " + NumberOfPanels + " Panels. If you are adding Panels dynamically during runtime, please update your pagination to reflect the number of Panels you will have before adding.", gameObject);
                        }
                        // Không set valid = false để cho phép chạy khi panels được tạo động
                    }
                }
                if (snapSpeed < 0)
                {
        ...
        private void Start()
        {
            ...
            {
                // Chỉ log warning thay vì throw exception để cho phép chạy khi panels được tạo động
                Debug.LogWarning("<b>[SimpleScrollSnap]</b> Invalid configuration. Setup will be skipped. Panels may be added dynamically during runtime.", gameObject);
            }
        }
        private void Update()
        {
